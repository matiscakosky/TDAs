...
void abb_iterar_post(abb_nodo_t* nodo,bool visitar(const char *, void *, void *),void* extra);
...
void llenar_item(abb_item_t* items, abb_nodo_t* nodo, size_t* n){
	items[*n].clave = nodo->clave;
	items[*n].valor = nodo->valor;
	*n += 1; 
}

void obtener_items(abb_item_t* items, abb_nodo_t* nodo, size_t* n){
	if (!nodo) return;
	obtener_items(items,nodo->izq,n);
	llenar_item(items,nodo,n);
	obtener_items(items,nodo->der,n);
}

abb_item_t* abb_obtener_items(const abb_t* abb){
	abb_item_t* items = malloc(sizeof(abb_item_t) * abb->cant);
	size_t cuenta = 0;
	obtener_items(items, abb->raiz, &cuenta);
	return items;
}
...
/*************************************** Iterador externo post order ABB ***************************************/

// Crear el iterador
abb_iter_post_t*  abb_iter_post_crear(const abb_t* abb){
...
bool  abb_iter_post_avanzar(abb_iter_post_t* iter){
	...
	abb_nodo_t* nodo = pila_desapilar(iter->pila);
	abb_nodo_t* aux = pila_ver_tope(iter->pila); //Neesito ver el nodo que quedo en la pila para seguir el post order
	if(aux->der && aux->der != nodo){
